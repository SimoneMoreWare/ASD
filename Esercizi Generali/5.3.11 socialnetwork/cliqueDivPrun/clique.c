...
graph leggi_file(FILE *in, TS ts) {
  ...
  while(fscanf(in, "%s %s %d", c, r, &w) == 3) {
    ...
    if (id1 < 0 || id2 < 0) printf("Errore in lettura\n");
  }
...
int check(graph g, int *sol, int nNodi) {
  ...
  for(i=0; i<v; i++) {
    ...
    if (c) return 0; /* Clique non massimale in quanto puo' essere estesa */
  }
...
void generaCliquesR(graph g, int *sol, cliques *clT, int pos, int start) {
  ...
  for (i=start; i<v; i++) {
    /* pruning: verifica se e' connessa ai nodi nella soluzione */
    int j, connesso = 1;
    for (j=0; j<pos; j++) {
      if (!GRAPHadiacenti(g, sol[j], i)) {
        connesso = 0;
        break;
      }
    }
