...
/* Esercitazione 9
Le sequenze sono indicizzate da [a, b), con a incluso e b escluso
*/
...
int SolRic1(int *frecce, int *best_sol, int n) {
  ...
  s = calloc((n/2+1), sizeof(int *));
  for (i = 0; i < n/2+1; i++) {
    s[i] = calloc((n/2+1), sizeof(int));
  }
  res = minCostR1(frecce, s, 0, n, INT_MAX, &nCalls);
  printf ("Soluzione ricorsiva: %d chiamate\n", nCalls);
  displaySol(s, best_sol, 0, n);
  for (i = 0; i < n/2+1; i++) {
    free(s[i]);
  }
  free(s);
...
int minCostR1(int *frecce, int **s, int i, int j, int minCost, int *nCalls) {
  ...
  s[i/2][j/2] = j;
  for (k=i+2; k<=j-2; k=k+2) {
    cost = minCostR1(frecce, s, i, k, minCost, nCalls) + minCostR1(frecce, s, k, j, minCost, nCalls);
    if (cost < minCost) {
      minCost = cost;
      s[i/2][j/2] = k;
    }
...
/* Ricorsione + memoization (top-down) */
int SolDinMemo(int *frecce, int *best_sol, int n) {
  ...
  for (i = 0; i < n/2+1; i++) {
    m[i] = malloc((n/2+1)*sizeof(int));
    for (j = 0; j < n/2+1; j++)
      m[i][j] = -1;
  }
  res = minCostR2(frecce, s, m, 0, n, INT_MAX, &nCalls);
...
int minCostR2(int *frecce, int **s, int **m, int i, int j, int minCost, int *nCalls) {
  ...
  s[i/2][j/2] = j;
  for (k=i+2; k<=j-2; k=k+2) {
    cost = minCostR2(frecce, s, m, i, k, minCost, nCalls) + minCostR2(frecce, s, m, k, j, minCost, nCalls);
    if (cost < minCost) {
      minCost = cost;
      s[i/2][j/2] = k;
    }
...
int SolProgrDin(int *frecce, int n) {
  ...
  m = calloc((n/2+1), sizeof(int *));
  s = calloc((n/2+1), sizeof(int *));
  sol = calloc(n, sizeof(int));
  for (i = 0; i < n/2+1; i++) {
    m[i] = calloc((n/2+1), sizeof(int));
    s[i] = calloc((n/2+1), sizeof(int));
  }
  for (l = 2; l <= n; l+=2) {
    for (i = 0, j = i + l; j <= n; i+=2, j+=2) {
      m[i/2][j/2] = contaCambi(frecce, i, j);
      s[i/2][j/2] = j;
      for (k = i+2; k <= j-2; k=k+2) {
	q = m[i/2][k/2] + m[k/2][j/2];
	if (q < m[i/2][j/2]) {
	  m[i/2][j/2] = q;
	  s[i/2][j/2] = k;
	}
